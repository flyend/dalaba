...
!(function(global) {
    ...
    var Dalaba = (function() {
        ...
        Dalaba.Numeric = (function() {
            ...
            var notNULL = function(data) {
                ...
                while (++i < n)
                    if (isString(data[i]) || isNumber(data[i], true)) d.push(data[i]);
            ...
            var lerpArray = function(oldArrays, newArrays, timer, that) {
                ...
                newArrays.forEach(function(newValue, i) {
                    ...
                    if (isObject(newValue)) {
                        for (p in newValue)
                            if (hasOwnProperty.call(newValue, p)) {
                                newProps = newValue[p];
                                oldProps = oldValue[p];
                                if (isArray(newProps)) {
                ...
                onframe: function(timer) {
                    ...
                    function caller(oldProps, newProps, that) {
                        for (var p in newProps)
                            if (hasOwnProperty.call(newProps, p)) {
                                //newProps = oldProps
                                newValue = newProps[p];
                                oldValue = oldProps[p];
            ...
            var addTooltip = function() {
                ...
                var tooltipHide = function(chart, e, pos) {
                    ...
                    var callback = function() {
                        ...
                        pos !== null && clearBuffer(chart, extend({}, e, {
                            moveX: pos.x,
                            moveY: pos.y,
                            type: "mousemove"
                        }));
                    ...
                    if (!linked) {
                        chart.panel.forEach(function(pane) {
                            ...
                            if (tooltip && !Intersection.rect({
                                    ...
                                    x: pane.x,
                                    y: pane.y,
                                    width: pane.x + pane.width,
                                    height: pane.y + pane.height
                                })) {
            ...
            var onClick = function(e, chart, selector) {
                ...
                if (isClicking && chart.globalAnimation.isReady === true) {
                    ...
                    for (i = 0; i < graphics.length; i++) {
                        graphic = graphics[i];
                        for (j = 0; j < graphics[i].series.length; j++)
                            if ((series = graphic.series[j]).selected !== false) {
                                ...
                                if (points.length) {
                                    ...
                                    callbacks.push([!!points.length && isFunction(click),
                                        points.map(function(shape) {
                                            return shape.shape;
                                        }),
                                        click
                                    ]);
                    ...
                    if (globalClick) {
                        globalClick.call(points, extend({}, e, {
                            points: points,
                            shapes: points,
                            moveX: x,
                            moveY: y
    ...
    var setAttribute = function(domel, props) {
        if (domel)
            for (var p in props)
                if (hasOwnProperty.call(props, p) && defined(props[p]))
                ...
                var ret = {
                    ...
                    call: function(shape, series, context) {
                        ...
                        if (shape.selected !== false && series.selected !== false && options.enabled === true && defined(value)) {
                            ...
                            if (useHTML === true) {
                                setStyle(tag, {
                                    left: x + bbox.width / 2 * (dataLabel.align === "left") + "px",
                                    top: y - bbox.height + "px",
                                    visibility: "visible"
                                });
                ...
                if (defined(item)) {
                    if (defined(item.style)) {
                        for (var p in chartOptions.style)
                            if (!({}).hasOwnProperty.call(item.style, p)) {
        ...
        chartProto = {
                            ...
                            if ((series.type === "pie" || series.type === "funnel") && series.showInLegend === true) {
                                data.forEach(function(item, j) {
                                    if (item !== null && (isObject(item) && item.value !== null)) {
                                        ...
                                        seriesData.push({
                                            type: series.type,
                                            /*seriesIndex: i, dataIndex: j, */ disabled: true
                                        });
                                    }
                                });
                            } else {
            ...
            addPlotSeries: function(types) {
                ...
                this.series.forEach(function(series, i) {
                    ...
                    types && (types[type] = {
                        type: type,
                        weight: pack("number", series.zIndex, i + 1)
                    });
                });
            ...
            addOverlap: function() {
                ...
                this.series.forEach(function(series) {
                    ...
                    if (defined(dataLabels) && dataLabels.enabled !== false) {
                        ...
                        if (useHTML) {
                            ...
                            isString(dataLabels.className) && setAttribute(domLabel, {
                                "class": dataLabels.className
                            });
                            setStyle(domLabel, {
                                position: "absolute",
                                top: "0px",
                ...
                this.globalHTML.dataLabels = domDataLabels.map(function(d) {
                    return d.dom;
                });
    ...
    (function(global, Dalaba) {
        ...
        Axis.prototype = {
                ...
                tick.plotLines = function() {
                    ...
                    var caller = {
                        xAxis: [
                            ...
                            function(p) {
                                return mathFloor(p.x - 0.5) <= mathCeil(maxRange + tx) && mathCeil(p.x + 0.5) >= mathFloor(tx);
                            }, // inside bound
                            function(p0, lineWidth) {
                                p0 = center.call(axis, p0, lineWidth);
                                return [p0, ty - options.height, p0, ty, lineWidth];
                            },
                            [tx, tx + maxRange]
                        ],
                        yAxis: [
                            ...
                            function(p) {
                                return mathFloor(p.y - 0.5) <= mathCeil(maxRange + ty) && mathCeil(p.y - 0.5) >= mathFloor(ty);
                            },
                            function(p0, lineWidth) {
                                return [tx, p0, tx + options.width, p0, lineWidth];
                            },
                            [maxRange + ty, ty]
                        ]
                    }[axis.name];
                    ...
                    if (caller && plotLines.length) {
                        notPlotlines.forEach(function(d, i) {
                            linePixel = fixLinePixel.apply(null, caller[3](interpolate.apply(null, [i, 0, ~-notPlotlines.length].concat(caller[4])), lineWidth));
                            addPlotline(d, linePixel);
                        });
                        plotlines.forEach(function(d, i) {
                            if (values.length) {
                                linePixel = fixLinePixel.apply(null, caller[3](interpolate.apply(null, [d.defaultZero === true ? 0 : d.value, values[0], values[~-values.length]].concat(caller[4])), lineWidth));
                                addPlotline(d, linePixel);
                            }
                        });
                    }
            ...
            getCategoriesTicks: function(startValue, endValue, maxLength) {
                ...
                if (isNumber(tickAmount, true) && (tickAmount = mathMax(1, ~~tickAmount)))
            ...
            setLabels: function() {
                ...
                var ticks = arrayFilter(this.ticks, function(tick) {
                        return tick.visibled;
                    }),
                    type = this.type,
                    name = this.name;
                var size = 0,
            ...
            setCrosshair: function(x, y) {
                ...
                if (defined(crosshair) && isNumber(x, true) && isNumber(y, true) && n) {
                    ...
                    for (i = 1; i < n; i++) {
                        ...
                        if (intersected[this.name] && intersected[this.name]({
                                x: x,
                                y: y
                            }, tickPositioner[i - 1], tick)) {
                            this.crosshair = {
                                zIndex: 2,
                                shape: tick
                            };
            ...
            animateTo: function() {
                ...
                var oldData = this._ticks,
                    newData = arrayFilter(this.ticks, function(tick) {
                        return tick.visibled;
                    });
                var axis = this;
                var ticks = [];
        ...
        Line.prototype = {
            ...
            animateTo: function(initialize) {
                ...
                this.series.forEach(function(series) {
                    ...
                    }).add(function(newIndex) {}).remove(function(newIndex) {
    ...
    (function(global, Chart) {
        ...
        Column.prototype = {
            ...
            drawShape: function(context, shape, series) {
                ...
                context.save();
                borderRadius = TRouBLe(borderRadius).map(function(d) {
                    return d * 2;
                });
                context.beginPath();
            ...
            getShape: function(x, y, shared) {
                ...
                for (var k = 0; k < groups.length; k++) {
                    ...
                    for (var i = 0; i < groups[k].length; i++)
                        if ((item = group[i]).selected !== false) {
                            plotY = item.plotY;
    ...
    (function(global, Chart) {
        ...
        var relayout = (function(global) {
            ...
            var addDataLabels = function(shapes, series) {
                ...
                var centerX = shapeArgs.x,
                    centerY = shapeArgs.y, //L=x
                    radius = shapeArgs.radius,
                    innerRadius = shapeArgs.innerRadius;
                ...
                return function(panels, isResized) {
                    panels.forEach(function(pane) {
                        pane.series.forEach(function(series) {
                            ...
                            while (++j < length)
                                if (!(shape = shapes[j]).isNULL && shape.selected !== false) sumValue += shape.value;
                            ...
                            if ((projection === "geo" || isObject(projection)) && defined(series.seriesTarget)) {
                                ...
                                if (seriesTarget !== null) {
                                    ...
                                    if (defined(seriesTarget.__projector__)) {
                                        projection = function(point) {
                                            return seriesTarget.__projector__.projection(point);
                                        };
                                    }
                                }
    ...
    (function(global, Chart) {
        ...
        var relayout = (function(global) {
            ...
            var neckWidth = function(series, plotX, plotY, plotWidth, plotHeight) {
                ...
                while (++j < length)
                    if ((shape = shapes[j]).selected === false && isNumber(value = shape.value, true) && value > 0) sumValue -= value;
                j = reversed ? length : -1;
                var al = 0;
                        ...
                        pane.series.forEach(function(series, i) {
                            ...
                            for (; j < length; j++) {
                                ...
                                extend(shape, {
                                    ...
                                    y: y,
                                    x1: x1,
                                    y1: y1, // (InterQuartile Range, IQR) Box height
                                    x2: x2,
                                    y2: y2, // median
                                    x3: x3,
                                    y3: y3, // lower
            ...
            getShape: function(x, y, shared) {
                ...
                for (i = 0; i < sl; i++) {
                    ...
                    for (j = 0, dl = shapes.length; series.selected !== false && series.enableMouseTracking !== false && j < dl; j++) {
                        ...
                        flag = shape.isNULL;
                        while (!flag && intersectedPoints.length)
                            if (flag = (Intersection.segment.apply(null, [{
                                    x: x,
                                    y: y
                                }].concat(intersectedPoints.pop())))) {
                                shape.current = j;
                                results.push({
                                    shape: shape,
                                    series: series
                ...
                for (i = 0; i < sl; i++) {
                    ...
                    for (j = 0, dl = shapes.length; series.selected !== false && j < dl; j++) {
                        ...
                        if (Intersection.aabb({
                                ...
                                x: shape.x,
                                y: shape.y,
                                width: mathAbs(shape.x - shape.x1),
                                height: mathAbs(shape.y - shape.y1)
                            })) {
                            shape.selected = j;
                            results.push({
                                shape: shape,
